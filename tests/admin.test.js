...
(async () => {
    ...
    await p.screenshot({ path: path.join(__dirname, 'shots', '20-admin.png'), fullPage: false });
    await p.evaluate(() => { window.PTD.set('preset', 'forest'); });
    await p.waitForTimeout(400);
    await p.screenshot({ path: path.join(__dirname, 'shots', '21-admin-forest.png') });
    await p.evaluate(() => { window.PTD.set('mode', 'light'); window.PTD.set('preset', 'ocean'); });
    await p.waitForTimeout(400);
    await p.screenshot({ path: path.join(__dirname, 'shots', '22-admin-light.png') });
